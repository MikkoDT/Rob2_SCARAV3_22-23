from tkinter import *
from tkinter import messagebox
from tkinter import PhotoImage
import numpy as np
import math
import roboticstoolbox as rtb
from roboticstoolbox import DHRobot, RevoluteDH, PrismaticDH
import spatialmath
from spatialmath import SE3
import matplotlib
matplotlib.use('TkAgg')

#Create a GUI Window with a tittle
mygui = Tk()
mygui.title("Spherical Calculator")
mygui.resizable(False,False)
mygui.configure(bg="blue")

def reset():
    a1_E.delete(0, END)
    a2_E.delete(0, END)
    a3_E.delete(0, END)

    T1_E.delete(0, END)
    T2_E.delete(0, END)
    d3_E.delete(0, END)

    x_E.delete(0, END)
    y_E.delete(0, END)
    z_E.delete(0, END)

def f_k():
    # link lengths in cm
    a1 = float(a1_E.get())/100
    a2 = float(a2_E.get())/100
    a3 = float(a3_E.get())/100

    # Joint Variables: mm if d and degrees if T
    T1 = float(T1_E.get())
    T2 = float(T2_E.get())
    d3 = float(d3_E.get())/100
    
    # deg to rad
    T1 = (T1*np.pi)/180.0
    T2 = (T2*np.pi)/180.0

    # Parametric Table (Theta ,alpha, r, d)
    PT = [[T1, (90.0*np.pi)/180.0, 0,a1], 
         [((90.0*np.pi)/180.0)+T2, (90.0*np.pi)/180.0,0,0], 
         [(0.0*np.pi)/180.0, (0.0*np.pi)/180.0, 0, a2+a3+d3]]

    # HTM Formulae
    i = 0
    H0_1 = [[np.cos(PT[i][0]),-np.sin(PT[i][0])*np.cos(PT[i][1]), np.sin(PT[i][0])*np.sin(PT[i][1]), PT[i][2]*np.cos(PT[i][0])], 
            [np.sin(PT[i][0]),np.cos(PT[i][0])*np.cos(PT[i][1]),-np.cos(PT[i][0])*np.sin(PT[i][1]),PT[i][2]*np.sin(PT[i][0])], 
            [0,np.sin(PT[i][1]),np.cos(PT[i][1]),PT[i][3]], 
            [0,0,0,1]]

    i = 1
    H1_2 = [[np.cos(PT[i][0]),-np.sin(PT[i][0])*np.cos(PT[i][1]), np.sin(PT[i][0])*np.sin(PT[i][1]), PT[i][2]*np.cos(PT[i][0])], 
            [np.sin(PT[i][0]),np.cos(PT[i][0])*np.cos(PT[i][1]),-np.cos(PT[i][0])*np.sin(PT[i][1]),PT[i][2]*np.sin(PT[i][0])], 
            [0,np.sin(PT[i][1]),np.cos(PT[i][1]),PT[i][3]], 
            [0,0,0,1]]

    i = 2
    H2_3 = [[np.cos(PT[i][0]),-np.sin(PT[i][0])*np.cos(PT[i][1]), np.sin(PT[i][0])*np.sin(PT[i][1]), PT[i][2]*np.cos(PT[i][0])], 
            [np.sin(PT[i][0]),np.cos(PT[i][0])*np.cos(PT[i][1]),-np.cos(PT[i][0])*np.sin(PT[i][1]),PT[i][2]*np.sin(PT[i][0])], 
            [0,np.sin(PT[i][1]),np.cos(PT[i][1]),PT[i][3]], 
            [0,0,0,1]]

    H0_1 = np.matrix(H0_1)
    H1_2 = np.matrix(H1_2)
    H2_3 = np.matrix(H2_3)

    H0_2 = np.dot(H0_1,H1_2)
    H0_3 = np.dot(H0_2,H2_3)

    x0_3 = H0_3[0,3]
    x_E.delete(0,END)
    x_E.insert(0,np.around(x0_3*100,3))

    y0_3 = H0_3[1,3]
    y_E.delete(0,END)
    y_E.insert(0,np.around(y0_3*100,3))

    z0_3 = H0_3[2,3]
    z_E.delete(0,END)
    z_E.insert(0,np.around(z0_3*100,3))

    # Create links
# [robot variable]=DHRobot([RevoluteDH(d,r,alph,offset)])
    Spherical = DHRobot([
        RevoluteDH(a1,0,(90.0*np.pi)/180.0,(0.0*np.pi)/180.0,qlim=[-np.pi/2,np.pi/2]),
        RevoluteDH(0,0,(90.0*np.pi)/180.0,(90.0*np.pi)/180.0,qlim=[-np.pi/2,np.pi/2]),
        PrismaticDH(0,0,(0.0*np.pi)/180.0, a2+a3,qlim=[0,30/100]),

    ], name='Spherical')

    # plot joints
    q1 = np.array([T1,T2,d3])

    # plot scale
    x1 = -0.5
    x2 = 0.5
    y1 = -0.5
    y2 = 0.5
    z1 = 0.0
    z2 = 0.5

    # plot command
    Spherical.plot(q1,limits=[x1,x2,y1,y2,z1,z2],block=True)

def i_k():
    #Inverse Kinematics USing Graphical Method

    # link lengths in cm
    a1 = float(a1_E.get())
    a2 = float(a2_E.get())
    a3 = float(a3_E.get())

    # Position Vectors in cm
    xe = float(x_E.get())
    ye = float(y_E.get())
    ze = float(z_E.get())

    # To solve for THeta 2o or Th2
    # Try & except
    try:
        Th1 = np.arctan(ye/xe) #1
    except:
        Th1 = -1 #NAN error
        messagebox.showerror(title="DividedbyZero Error", message="Undefined Solution if X=0.")

    # To solve for Theta 1 or TH1
    Th1 = np.arctan(ye/xe) #1

    # To solve for Theta 2 or TH2
    r1 = np.sqrt(xe**2 + ye**2) #2
    r2 = ze - a1 #3
    Th2 = np.arctan(r2/r1) #4

    # To solve for D3
    D3 = (np.sqrt(r1**2 + r2**2) - a2 - a3) #5

    T1_E.delete(0,END)
    T1_E.insert(0,np.around(Th1*180/np.pi,3))

    T2_E.delete(0,END)
    T2_E.insert(0,np.around(Th2*180/np.pi,3))

    d3_E.delete(0,END)
    d3_E.insert(0,np.around(D3,3))

    # Create links
    # [robot variable]=DHRobot([RevoluteDH(d,r,alph,offset)])
    Spherical = DHRobot([
        RevoluteDH(a1/100,0,(90.0*np.pi)/180.0,(0.0*np.pi)/180.0,qlim=[-np.pi/2,np.pi/2]),
        RevoluteDH(0,0,(90.0*np.pi)/180.0,(90.0*np.pi)/180.0,qlim=[-np.pi/2,np.pi/2]),
        PrismaticDH(0,0,(0.0*np.pi)/180.0, a2/100+a3/100,qlim=[0,30/100]),

    ], name='Spherical')                                   

    # plot joints
    q1 = np.array([Th1,Th2,D3/100])

    # plot scale
    x1 = -0.5
    x2 = 0.5
    y1 = -0.5
    y2 = 0.5
    z1 = 0.0
    z2 = 0.5

    # plot command
    Spherical.plot(q1,limits=[x1,x2,y1,y2,z1,z2],block=True)

# Link lengths and Joint Variable frames
FI = LabelFrame(mygui,text="Link Lengths and Joint Variables",font=(5))
FI.grid(row=1,column=0)

# label of link lengths
a1 = Label(FI,text=("a1 ="),font=(10))
a1_E = Entry(FI,width=5,font=(10))
cm1 = Label(FI,text=("cm"),font=(10))

a2 = Label(FI,text=("a2 ="),font=(10))
a2_E = Entry(FI,width=5,font=(10))
cm2 = Label(FI,text=("cm"),font=(10))

a3 = Label(FI,text=("a3 ="),font=(10))
a3_E = Entry(FI,width=5,font=(10))
cm3 = Label(FI,text=("cm"),font=(10))

a1.grid(row=1,column=1)
a1_E.grid(row=1,column=2)
cm1.grid(row=1,column=3)

a2.grid(row=2,column=1)
a2_E.grid(row=2,column=2)
cm2.grid(row=2,column=3)

a3.grid(row=3,column=1)
a3_E.grid(row=3,column=2)
cm3.grid(row=3,column=3)

# label of Joint Variables
T1 = Label(FI,text=("T1 ="),font=(10))
T1_E = Entry(FI,width=5,font=(10))
deg1 = Label(FI,text=("deg"),font=(10))

T2 = Label(FI,text=("T2 ="),font=(10))
T2_E = Entry(FI,width=5,font=(10))
deg2 = Label(FI,text=("deg"),font=(10))

d3 = Label(FI,text=("d3 ="),font=(10))
d3_E = Entry(FI,width=5,font=(10))
cm4 = Label(FI,text=("cm"),font=(10))

T1.grid(row=1,column=4)
T1_E.grid(row=1,column=5)
deg1.grid(row=1,column=6)

T2.grid(row=2,column=4)
T2_E.grid(row=2,column=5)
deg2.grid(row=2,column=6)

d3.grid(row=3,column=4)
d3_E.grid(row=3,column=5)
cm4.grid(row=3,column=6)

# Buttons frame
BF = LabelFrame(mygui,text="Forward Kinematics",font=(5))
BF.grid(row=3,column=0)

# Buttons label
Fk = Button(BF,text="↑Forward",font=(10),bg="green",fg="white",command=f_k)
rst = Button(BF,text="RESET",font=(10),bg="yellow",fg="white",command=reset)
Ik = Button(BF,text="↓Inverse",font=(10),bg="red",fg="white",command=i_k)

Fk.grid(row=0,column=0)
rst.grid(row=0,column=1)
Ik.grid(row=0,column=2)

# Position Vectors frames
PV = LabelFrame(mygui,text="Position Vectors",font=(5))
PV.grid(row=2,column=0)

# Position Vectors labels
x = Label(PV,text=("X ="),font=(10))
x_E = Entry(PV,width=5,font=(10))
cm5 = Label(PV,text=("cm"),font=(10))

y = Label(PV,text=("Y ="),font=(10))
y_E = Entry(PV,width=5,font=(10))
cm6 = Label(PV,text=("cm"),font=(10))

z = Label(PV,text=("Z ="),font=(10))
z_E = Entry(PV,width=5,font=(10))
cm7 = Label(PV,text=("cm"),font=(10))

x.grid(row=0,column=0)
x_E.grid(row=0,column=1)
cm5.grid(row=0,column=2)

y.grid(row=1,column=0)
y_E.grid(row=1,column=1)
cm6.grid(row=1,column=2)

z.grid(row=2,column=0)
z_E.grid(row=2,column=1)
cm7.grid(row=2,column=2)

# display image
img = PhotoImage(file="Spherical.png")
img = img.subsample(6,5)
PI = Label(mygui,image=img)
PI.grid(row=0,column=0)

mygui.mainloop() 