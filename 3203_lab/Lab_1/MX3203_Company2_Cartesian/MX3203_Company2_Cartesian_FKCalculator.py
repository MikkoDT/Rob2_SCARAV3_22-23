...
def f_k():
    ...
    [np.sin(PT[0][0]), np.cos(PT[i][0]*np.cos(PT[i][1])), -np.cos(PT[i][0])*np.sin(PT[i][1]), PT[i][2]*np.sin(PT[i][0])], 
...
# [robot_variable]=DHRobot([RevoluteDH(d, r, alpha, offset)])
    ...
    PrismaticDH(0,0,(270.0/180.0)*np.pi,a1,qlim=[0,(30/100)]), 
    PrismaticDH((270.0/180.0)*np.pi,0,(270.0/180.0)*np.pi,a2,qlim=[0,(30/100)]),
    PrismaticDH((270.0/180.0)*np.pi,0,(90.0/180.0)*np.pi,a3,qlim=[0,(30/100)]),
    PrismaticDH(0,0,(0.0/180.0)*np.pi,a4,qlim=[0,(30/100)])],
    name= 'CARTESIANC2_03' )

# plot joints
    q1 = np.array([d1,d2,d3,0])
...
# Buttons Frame
BF = LabelFrame(mygui,text="Forward Kinematics",font=(5), bg="orange")
