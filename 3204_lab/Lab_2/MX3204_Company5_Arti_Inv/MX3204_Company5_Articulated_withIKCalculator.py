...
mygui.configure(bg = "spring green")
...
def f_k():
    ...
    i = 0
    H0_1 = [[np.cos(PT[i][0]),-np.sin(PT[i][0])*np.cos(PT[i][1]),
np.sin(PT[i][0])*np.sin(PT[i][1]), PT[i][2]*np.cos(PT[i][0])],
[np.sin(PT[i][0]), np.cos(PT[i][0])*np.cos(PT[i][1]), -np.cos(PT[i][0])*np.sin(PT[i][1]),
PT[i][2]*np.sin(PT[i][0])],
[0, np.sin(PT[i][1]), np.cos(PT[i][1]), PT[i][3]],
[0, 0, 0, 1]]
    
    i = 1
    H1_2 = [[np.cos(PT[i][0]),-np.sin(PT[i][0])*np.cos(PT[i][1]),
np.sin(PT[i][0])*np.sin(PT[i][1]), PT[i][2]*np.cos(PT[i][0])],
[np.sin(PT[i][0]), np.cos(PT[i][0])*np.cos(PT[i][1]), -np.cos(PT[i][0])*np.sin(PT[i][1]),
PT[i][2]*np.sin(PT[i][0])],
[0, np.sin(PT[i][1]), np.cos(PT[i][1]), PT[i][3]],
[0, 0, 0, 1]]
    ...
    i = 2
    H2_3 = [[np.cos(PT[i][0]),-np.sin(PT[i][0])*np.cos(PT[i][1]),
np.sin(PT[i][0])*np.sin(PT[i][1]), PT[i][2]*np.cos(PT[i][0])],
[np.sin(PT[i][0]), np.cos(PT[i][0])*np.cos(PT[i][1]), -np.cos(PT[i][0])*np.sin(PT[i][1]),
PT[i][2]*np.sin(PT[i][0])],
[0, np.sin(PT[i][1]), np.cos(PT[i][1]), PT[i][3]],
[0, 0, 0, 1]]
    ...
    ARTICULATED = DHRobot([RevoluteDH (a1, 0, (90.0/180.0)*np.pi, (0.0/180.0)*np.pi, qlim=[-np.pi/2, np.pi/2]),RevoluteDH (0, a2, (0.0/180.0)*np.pi, (0.0/180.0)*np.pi, qlim=[-np.pi/2, np.pi/2]),RevoluteDH (0, a3, (0.0/180.0)*np.pi, (0.0/180.0)*np.pi, qlim=[-np.pi/2, np.pi/2])], name = "ARTICULATED")
...
def i_k():
    ...
    try:
        ...
        Th1 = -1 # Meaning: NAN error messagebox.showerror(title = "DividedbyZero Error", message = "Undefined Solution if X= 0")
    ...
    # Joint Variables
    T1_E.delete(0, END)
    T1_E.insert(0, np.around(Th1*180.0/np.pi, 3))

    T2_E.delete(0, END)
    T2_E.insert(0, np.around(Th2*180.0/np.pi, 3))

    T3_E.delete(0, END)
    T3_E.insert(0, np.around(Th3*180.0/np.pi, 3))

    # Create Links
    # [robot_variable]= DHRobot([RevoluteDH(d,r,alpha,offset)])
    ARTICULATED = DHRobot([RevoluteDH (a1/100, 0, (90.0/180.0)*np.pi, (0.0/180.0)*np.pi, qlim=[-np.pi/2, np.pi/2]),RevoluteDH (0, a2/100, (0.0/180.0)*np.pi, (0.0/180.0)*np.pi, qlim=[-np.pi/2, np.pi/2]),RevoluteDH (0, a3/100, (0.0/180.0)*np.pi, (0.0/180.0)*np.pi, qlim=[-np.pi/2, np.pi/2])], name = "ARTICULATED")
...
FI = LabelFrame(mygui, text="Link Lengths and Joint Variables", font = (5), labelanchor = "n",bg = "lime green", fg = "magenta")
...
# Buttons Frame
BF = LabelFrame(mygui, text = "Forward Kinematics", font = 'Arial 12 ', labelanchor = "n",bg = "lime green", fg = "magenta")
BF.grid(row = 1, column = 0)

#Buttons
FK = Button(BF, text = "↑ Forward", font = (10), bg = "blue", fg = "white", command = f_k)
rst = Button(BF, text = "RESET", font = (10), bg = "violet", fg = "white", command = reset)
...
PV = LabelFrame(mygui, text = "Position Vectors", font=(5), bg = "lime green", fg = "magenta")
