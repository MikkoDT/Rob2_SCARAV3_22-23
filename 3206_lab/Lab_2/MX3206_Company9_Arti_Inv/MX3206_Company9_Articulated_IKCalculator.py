...
def f_k():
    ...
    # joint variables: degrees if theta
    T1 = float(T1_E.get())
    T2 = float(T2_E.get())
    T3 = float(T3_E.get())
    ...
    PT = [[T1, (90.0/180.0)*np.pi,0,a1],
          [T2, (0/180.0)*np.pi,a2,0],
          [T3, (0/180.0)*np.pi,a3,0]]
    ...
    ARTICULATED_V3 = DHRobot([
        RevoluteDH(a1,0,(90.0/180.0)*np.pi,(0/180.0)*np.pi,qlim=[-np.pi/2,np.pi/2]),
        RevoluteDH(0,a2,(0/180.0)*np.pi,(0/180.0)*np.pi,qlim=[-np.pi/2,np.pi/2]),
        RevoluteDH(0,a3,(0/180.0)*np.pi,(0/180.0)*np.pi,qlim=[-np.pi/2,np.pi/2]),
        ], name="ARTICULATED_V3")
    ...
    # plot command
    ARTICULATED_V3.plot(q1,limits=[x1,x2,y1,y2,z1,z2],block=True) 

def i_k():
    # Inverse Kinematics Solutions using Graphical Method
    ...
    # [robot_Variable]=DHRobot([RevoluteDH(d,r,alpha,offset)]
    ARTICULATED_V3 = DHRobot([
        RevoluteDH(a1/100,0,(90.0/180.0)*np.pi,(0/180.0)*np.pi,qlim=[-np.pi/2,np.pi/2]),
        RevoluteDH(0,a2/100,(0/180.0)*np.pi,(0/180.0)*np.pi,qlim=[-np.pi/2,np.pi/2]),
        RevoluteDH(0,a3/100,(0/180.0)*np.pi,(0/180.0)*np.pi,qlim=[-np.pi/2,np.pi/2]),
        ], name="ARTICULATED_V3") 
...
IK = Button(BF,text="↑ Inverse",font=(10),bg="yellow",fg="white",command=i_k)
