...
def inverse():
    ...
    if xe != 0:
        ...
        if -2 * r1 * a2 == 0:
          phi1 = np.pi/2 if a4**2 - r1**2 - a2**2 > 0 else -np.pi/2
        else:
            phi1 = np.arccos((a4**2 - r1**2 - a2**2) / (-2 * r1 * a2)) #3
        Th1 = phi2 - phi1 #4
    ...
    SCARA = DHRobot([
        ...
        PrismaticDH(a5/100,0,(0.0/180.0)*np.pi,(0.0/180.0)*np.pi,qlim=[0,(30/100)]),
        ], name="SCARA")
...
IK = Button(BF,text="↑Inverse",font=(15),bg="darkorchid",fg="ghostwhite",command=inverse)

FK.grid(row=0,column=0)
RST.grid(row=0,column=1)
IK.grid(row=0,column=3)
