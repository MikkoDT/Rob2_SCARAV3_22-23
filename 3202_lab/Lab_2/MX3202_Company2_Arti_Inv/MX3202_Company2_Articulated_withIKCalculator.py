...
# Window and Title
mygui = Tk()
mygui.title("ARTICULATEDV3 CALCULATOR")
mygui.resizable(False,False)
mygui.configure(bg="Yellow")
...
def f_k():
    ...
    H0_1 = [[np.cos(PT[i][0]),-np.sin(PT[i][0])*np.cos(PT[i][1]),np.sin(PT[i][0])*np.sin(PT[i][1]),PT[i][2]*np.cos(PT[i][0])],
            [np.sin(PT[i][0]),np.cos(PT[i][0])*np.cos(PT[i][1]),-np.sin(PT[i][0])*np.sin(PT[i][1]),PT[i][2]*np.sin(PT[i][0])],
    ...
    # Plot Command
    ARTICULATEDV3.plot(q1,limits=[x1,x2,y1,y2,z1,z2],block=True)
...
def i_k():
    ...
    try:
        ...
        Th1 = -1
        messagebox.showerror(title="DividedbyZeroError",message="Undefined Solution if X=0")

    # To solve Theta 1 or Th2
    Th1 = np.arctan(ye/xe) #1
    r1 = np.sqrt(ye**2 + xe**2) #2

    # To solve Theta 2 or Th2
    r2 = ze - a1 #3
    r3 = np.sqrt(r1**2 + r2**2) #4
    phi1 = np.arctan(r2/r1) #5
    phi2 = np.arccos((a3**2 - a2**2 - r3**2) / (-2 * a2 * r3)) #6
    Th2 = phi1 + phi2 #7

    # To solve Theta3 or Th3
...
# Link Lengths amd Joint Variable Frame
FI = LabelFrame(mygui,text="Link Lengths and Joint Variable",font=(5))
...
#Buttons
FK = Button(BF,text="↓ Forward",font=(10),bg="Violet",fg="white",command=f_k)
...
cm5.grid(row=2,column=2)

# Display Image
img = PhotoImage(file="Articulated.png")
img = img.subsample(4,2)
